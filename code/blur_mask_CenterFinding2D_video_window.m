function CC = blur_mask_CenterFinding2D_video_window(V,BackgroundFile,frames,th,sz,centerFile,BackgroundType,channel)
...
for kframe=frames
    ...
    if (kframe - windowEdge(windowCount)) >= 0 % we are on the next background window
        ...
        bg_rgb = Background(:,:,:,windowCount);
        mask = find(rgb2gray(bg_rgb) > mask_th);
        save(centerFile,'CC','th','sz','-v7.3','kframe','-append')
    end
